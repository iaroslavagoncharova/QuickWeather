...
function updateThemeClasses() {
    ...
    if (currentTheme === 'dark' || (currentHour >= darkModeStartHour || currentHour < darkModeEndHour)) {
...
// Call the setTheme function to initialize the theme
setTheme();
...
const updateSevenDayForecast = (data) => {
    ...
    weatherTemperatureMaxDivs.forEach((div, index) => {
        div.innerHTML = `Max. ${weatherTemperaturesMax[index]} ${currentUnits === 'celsius' ? '°C' : '°F'}`;
    });
    ...
    weatherTemperatureMinDivs.forEach((div, index) => {
        div.innerHTML = `Min. ${weatherTemperaturesMin[index]} ${currentUnits === 'celsius' ? '°C' : '°F'}`;
    });
    // adding rain
...
document.addEventListener('click', (event) => {
    if (!event.target.closest('#settings') && !event.target.matches('#customization-button')) {
        const settingsDiv = document.querySelector('#settings');
        settingsDiv.style.display = 'none';
    }
});
